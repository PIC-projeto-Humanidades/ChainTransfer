...
def routine():
    ...
    logger("⏳ Iniciando rotina recorrente a cada 10 segundos...")

    bundle_status_cache = {}

    while True:
        time.sleep(10)
        logger("🔍 Procurando bundles em nós ativos...")
        ...
        for node in unique_nodes:
            ...
            try:
                ...
                logger("⏳ Obtendo bundles em nós ativos...")
                res = requests.get(f"http://{ip}:3000/bundle/{secondary_hash}", timeout=(5, 65))
                logger("✅ Bundle em nós ativos recebido com sucesso")
                try:
                    bundle = res.json()
                except ValueError:
                    logger(f"❌ Resposta inválida de {ip}: {res.text[:100]}...")
                    continue

                bundle_hash = bundle.get("hash")
                files = bundle.get("bundle", [])
                if not bundle_hash or not isinstance(files, list):
                    logger(f"⚠️ Bundle inválido de {ip}, ignorando.")
                    continue
                ...
                for fname in initial_to_download:
                    if fname in existing:
                        ...
                        logger(f"ℹ️ Arquivo já existente em media_data: {fname} — marcando como recebido")
                    else:
                ...
                if set(files) <= downloaded:
                    ...
                    logger(f"✅ Todos os arquivos do bundle {bundle_hash} foram baixados.")

            except Exception as e:
                logger(f"❌ Erro ao processar o nó {ip}: {e}")
